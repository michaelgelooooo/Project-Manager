            ...
            <div className="relative card h-full bg-secondary/25 rounded-lg hover:bg-primary/75">
                <div className="card-body p-4 flex flex-col gap-2">
                    ...
                    <div className="flex items-start justify-between gap-2">
                        <div className="card-title text-base sm:text-lg leading-tight">{task.task_name}</div>
                        <span className={`badge badge-xs sm:badge-md font-semibold capitalize ${priorityStyles[task.priority] || 'badge-ghost'}`}>
                            <i className="fas fa-flag text-[10px]" />
                            {task.priority}
                        </span>
                    </div>
                    {/* Row 2: Project name + Due date */}
                    <div className="flex items-center justify-between gap-2 text-base-content">
                        <div className="flex items-center gap-1 min-w-0">
                            <i className="fas fa-folder text-[10px] shrink-0" />
                            <span className="text-xs sm:text-sm truncate">{task.project_name ?? '—'}</span>
                        </div>
                        {task.due_date ? (
