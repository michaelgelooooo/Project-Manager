    ...
    return (
        <Link to={`/projects/${project.slug}`} className="block">
            <div className={`relative h-full hover:scale-102 hover:-translate-y-1 transform transition-all duration-150 ease-in-out cursor-pointer`}>
                {/* Background card */}
                <div className={`absolute inset-0 bg-base-100 rounded-xl shadow-xl border-l-[8px] ${coverBorder[project.cover_image ?? 'default']}`}></div>

                {/* Foreground card */}
                <div className={`relative card h-full flex-row bg-secondary/25 rounded-xl hover:bg-primary/75 border-l-[8px] ${coverBorder[project.cover_image ?? 'default']}`}>
